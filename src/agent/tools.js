...
export const createBalancePositionTool = new DynamicStructuredTool({
  ...
  schema: {
    ...
    properties: {
      ...
      amount: {
        type: "number",
        description:
          "The amount of token X to deposit (in human-readable units, not lamports)",
      },
      skipPreflight: {
        type: "boolean",
  ...
  func: async ({
    ...
    try {
      console.log(
        "Creating balance position in pool with amount :",
        poolAddress,
        amount,
      );
      ...
      console.log(
        ...
        `https://explorer.solana.com/tx/${signature}?cluster=${connection.rpcEndpoint.includes("devnet") ? "devnet" : "mainnet-beta"}`,
      );
...
export const getPositionsTool = new DynamicStructuredTool({
  ...
  description:
    "Fetches all liquidity positions for the current user in DLMM pool",
  schema: {
    type: "object",
    properties: {
      verbose: {
...
export const removeLiquidityTool = new DynamicStructuredTool({
  ...
  description:
    "Removes liquidity from a DLMM position. Automatically fetches positions if not specified.",
  schema: {
    type: "object",
    properties: {
      positionIndex: {
        type: "number",
        description:
          "Index of position to modify (from get_positions), defaults to 0 (first position)",
        default: 0,
      },
      percentage: {
        type: "number",
        description:
          "Percentage of liquidity to remove (0-100), 100 = full close",
        minimum: 0,
        maximum: 100,
        default: 100,
      },
  ...
  func: async ({
    ...
    try {
      ...
      if (positionIndex >= userPositions.length) {
        throw new Error(
          `Position index ${positionIndex} out of range (found ${userPositions.length} positions)`,
        );
      }
